...
{
    public class UnitOfWork<TContext> : IRepositoryFactory, IUnitOfWork<AppDbContext>, IUnitOfWork where TContext : DbContext
    {
        ...
        public IAsyncRepository<T> GetRepository<T>(bool hasCustomRepository = false) where T : EntityBase
        {
            ...
            {
                var customRepo = _context.GetService<IAsyncRepository<T>>();
                if (customRepo != null)
                {
                    return customRepo;
                }
            ...
            {
                repositories[type] = new RepositoryBase<EntityBase>(_context);
            }
        ...
        public IQueryable<T> FromSql<T>(string sql, params object[] parameters) where T : class => _context.Set<T>().FromSqlRaw(sql, parameters);
